...
var existPointTeam=0;
var teamArray=[];
var ds=new ListView.DataSource({
  rowHasChanged:(row1,row2)=>row1!==row2,
});
export default class TeamsClass extends Component{
  constructor() {
    super();
    this.state = {
      dataSource:ds.cloneWithRows(teamArray),
    }
  }
  _DetailsSetArray(){
    existPointTeam=1;
     this.setState({
        dataSource:ds.cloneWithRows(singleToneDataManager.getInstance().getTeamsArray()),
     });

   }

    render(){
      if (singleToneDataManager.getInstance().getTeamsArray().length!==0&&existPointTeam==0){
